...
namespace ex10_MovieFinder2024
{
    ...
    {
        // Flag 값
        bool isFavorite = false; // 즐겨찾기인지 API로 검색한거지 false => openAPI, true => 즐겨찾기 보기
        ...
        private async void SearchMovie(string movieName)
        {
            ...
            //result string을 받아서 json(JObject)로 변경
            var jsonResult = JObject.Parse(result); // type.Parse(string형)
            var total = Convert.ToInt32(jsonResult["total_results"].ToString());
            //await this.ShowMessageAsync("검색수", total.ToString());
            var results = jsonResult["results"];
            var jsonArray = results as JArray;  // results가 json 배열이기 때문에 JArray로 변환 JArray는 List와 동일해서 foreach사용가능
            ...
            foreach (var item in jsonArray)
            {
                ...
                {
                    Adult = Boolean.Parse(item["adult"].ToString()), //Convert.ToBoolean(item["adult"]),
                ...
                // 앞에 있는 값은 프로퍼티 이기 때문에 대문자로 시작. 뒤에 있는 아이템의 값은 json 자체 키가 adult이기 때문에 소문자 그대로 적을 것
            }
        ...
        {
            if(e.Key == Key.Enter)  // 키보드 엔터키를 누르면
            {
        ...
        {   
            // 이미 검색결과가 나온 상태에서 재검색을 하면 데이터 그리드 결과가 바뀌면서 이 이벤트가 다시 발생
            try
            {
                ...
                if (string.IsNullOrEmpty(poster_path)) // 포스터 이미지가 없을 경우 -> No_Picture.png를 출력
                {
                    ImgPoster.Source = new BitmapImage(new Uri("/No_Picture", UriKind.RelativeOrAbsolute));
                }
                else
                {
        ...
        private async void BtnAddFavorite_Click(object sender, RoutedEventArgs e)
        {
            ...
            if(isFavorite == true)  // 즐겨찾기 보기를 한 뒤에 즐겨찾기 추가를 하려고 할 때 막는 것
            {
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(Helpers.Common.CONNSTRING))
                {
                    ...
                    {
                        // 체크쿼리를 이용하여 즐겨찾기 추가하려는 영화의 Id가 이미 DB상에 존재하는지 조회
                        SqlCommand chkCmd = new SqlCommand(MovieItem.CHECK_QUERY, conn);
                        chkCmd.Parameters.AddWithValue("@Id", item.Id);
                        var cnt = Convert.ToInt32(chkCmd.ExecuteScalar());  // COUNT(*) 등의 1row 1column 값을 리턴할 때는 ExecuteScalar 사용

                        if (cnt == 1) continue; // 이미 영화 Id가 DB 상에 존재할 경우 패스
                        ...
                        insRes += cmd.ExecuteNonQuery(); // 데이터 하나마다 INSERT QUERY 실행
                    }
                }
                ...
                {
                    await this.ShowMessageAsync("즐겨찾기", $"즐겨찾기 {addMovieItems.Count}건중 {insRes}건 저장 성공");
                }
            }
            catch (Exception ex)
            {
                ...
                using(SqlConnection conn = new SqlConnection(Helpers.Common.CONNSTRING))
                {
                    ...
                    foreach (DataRow row in dSet.Tables["MovieItem"].Rows)
                    {
                        var movieItem = new MovieItem()
                        {
                            ...
                            Reg_Data = Convert.ToDateTime(row["Reg_Date"])
                        };
        ...
        {
            if (isFavorite == false) // 검색상태인데 삭제하려고 하는 경우
            {
            ...
            try
            {
                ...
                BtnViewFavorite_Click(sender, e); // 즐겨찾기 보기를 재실행(새로고침)
            }
            catch (Exception ex)
            {
                throw;
